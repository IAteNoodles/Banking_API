...
def start_server(USERNAME, PASSWORD, DATABASE_NAME):
    import mariadb
    connector = mariadb.connect(user=USERNAME, password=PASSWORD)
...
def make_tables(connector, connection):
...
def populate(connector, connection):
    ...
    root_staff_id = randbytes(20).hex()
    ...
    for name in people_data:
        ...
        ROOT.add_user(people_data[name], user_id, password)
